"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.demoVerifyKnownSig = void 0;
const listr_1 = __importDefault(require("listr"));
const fs_1 = require("fs");
const crypto_1 = require("crypto");
const PREFIX = '30818902818100';
const APPENDIX = '0203010001';
const demoVerifyKnownSig = async () => {
    const tasks = new listr_1.default([
        {
            title: 'Read ID and data',
            task: async (ctx) => {
                ctx.binaryData = await fs_1.promises.readFile("./demo/data/data_bin_1.txt", "utf8");
                const publicModule1 = '';
                const publicModule2 = '';
                ctx.pk1Pem = '-----BEGIN RSA PUBLIC KEY-----\n' + Buffer.from(PREFIX + publicModule1 + APPENDIX, 'hex').toString('base64') + '\n-----END RSA PUBLIC KEY-----\n';
                ctx.pk2Pem = '-----BEGIN RSA PUBLIC KEY-----\n' + Buffer.from(PREFIX + publicModule2 + APPENDIX, 'hex').toString('base64') + '\n-----END RSA PUBLIC KEY-----\n';
                ctx.sig1 = '';
                ctx.sig2 = '';
            }
        },
        {
            title: 'Verify signature',
            task: async (ctx) => {
                ctx.verified1 = crypto_1.verify("sha256", Buffer.from(ctx.binaryData), crypto_1.createPublicKey({ key: ctx.pk1Pem, format: 'pem', type: 'pkcs1' }), Buffer.from(ctx.sig1.slice(2), 'hex'));
                ctx.verified2 = crypto_1.verify("sha256", Buffer.from(ctx.binaryData), crypto_1.createPublicKey({ key: ctx.pk2Pem, format: 'pem', type: 'pkcs1' }), Buffer.from(ctx.sig2.slice(2), 'hex'));
            }
        }
    ]);
    const ctx = await tasks.run();
    console.log(ctx.binaryData.length);
    console.log(ctx.verified1);
    console.log(ctx.verified2);
    return ctx.uniqueId;
};
exports.demoVerifyKnownSig = demoVerifyKnownSig;
//# sourceMappingURL=demoVerifyKnownSig.js.map